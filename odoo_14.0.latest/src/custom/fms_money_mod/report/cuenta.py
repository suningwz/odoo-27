     ...
     #   self.user_id.address_home_id.bank_ids = [ (0, 0, {'acc_number': self.Cuenta})]
      #  cedula = self.env["res.users"].search(
       #     [("name", "=", self.name)], limit=1
        #)
        # Busqueda para encontrar la tabla res.partner.bank del usuario y a su vez encontrar los numeros
        # que esten asociada a ella en la tabla de acc_number y mostrarlas en el campo de employee_bank_account_id =)
